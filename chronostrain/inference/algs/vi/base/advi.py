...
class AbstractADVISolver(AbstractModelSolver, AbstractADVI, ABC):
    ...
    def initialize_data(self, read_batch_size: int) -> List[List[np.ndarray]]:
        ...
        if batch_metadata.exists():
            ...
            return [
                [
                    np.load(subdir / f't_{t_idx}_singular_batch_{batch_idx}.npy')
                    for batch_idx in range(n_batches_per[t_idx].n_batches)
                ] + [
                    np.load(subdir / f't_{t_idx}_paired.npy')
                ]
    ...
    def compute_marginalization(self,
                                ...
                                target_dir: Path,
                                read_batch_size: int) -> List[List[np.ndarray]]:
        batches = [
        ...
        for t_idx in range(self.gaussian_prior.num_times):
            ...
            for batch_idx, data_t_batch in enumerate(
                ...
                np.save(str(target_dir / f't_{t_idx}_singular_batch_{batch_idx}.npy'), strain_batch_lls_t)
            ...
            np.save(str(target_dir / f't_{t_idx}_paired.npy'), strain_batch_lls_t)
            batches[t_idx].append(strain_batch_lls_t)
            total_sz_t += strain_batch_lls_t.shape[1]
            total_sizes[t_idx] = total_sz_t
