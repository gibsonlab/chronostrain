...
class ReparametrizedGaussianPosterior(AbstractReparametrizedPosterior, ABC):
    def __init__(self, num_strains: int, num_times: int, params: Optional[GENERIC_PARAM_TYPE] = None):
...
class GaussianPosteriorFullReparametrizedCorrelation(ReparametrizedGaussianPosterior):
    ...
    def initial_params(self) -> GENERIC_PARAM_TYPE:
        ...
        params = {
            'tril_weights': np.zeros((n_features * (n_features - 1)) // 2, dtype=self.dtype),
            'diag_weights': np.full(n_features, fill_value=cnp.log(INIT_SCALE), dtype=self.dtype),
        }

        if self.initial_gaussian_bias is None:
            params['bias'] = np.zeros(n_features, dtype=self.dtype)
        else:
            params['bias'] = np.ravel(self.initial_gaussian_bias)
        return params
    ...
    def reparametrize(self, random_samples: GENERIC_SAMPLE_TYPE, params: GENERIC_PARAM_TYPE) -> np.ndarray:
