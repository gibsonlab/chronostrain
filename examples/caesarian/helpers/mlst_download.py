...
def generate_mlst_db(mlst_xml_path: Path, target_genera: Optional[Set[str]]) -> PandasAssistedBackend:
    db = PandasAssistedBackend()
    for taxon, loci in parse_mlst_sequences(mlst_xml_path):
        ...
        if target_genera is None or taxon.genus in target_genera:
            ...
            db.add_strains(parse_mlst_types(taxon, loci))
    return db
...
def main():
    ...
    mlst_xml_path = out_path.with_suffix('.xml')
    ...
    db = generate_mlst_db(mlst_xml_path, target_genera)
    with open(out_path, 'wb') as f:
        pickle.dump(db, f)
    logger.debug("Saved database to {}".format(out_path))
