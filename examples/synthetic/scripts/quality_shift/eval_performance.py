...
def q_shift_dirs(base_dir: Path) -> Iterator[Tuple[int, Path]]:
    for child_dir in base_dir.glob("qshift_*"):
...
def main():
    ...
    df_entries = []
    for q_shift, q_shift_dir in q_shift_dirs(base_dir):
        for trial_num, trial_dir in trial_dirs(q_shift_dir):
            print(f"Handling qshift {q_shift}, trial {trial_num}")
