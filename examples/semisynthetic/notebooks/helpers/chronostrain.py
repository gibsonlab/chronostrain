...
def extract_chronostrain_prediction(
    ...
    output_dir = output_basedir / 'chronostrain'
    ...
    runtime = parse_runtime(output_basedir / 'chronostrain_filter_runtime.txt') + parse_runtime(
        output_basedir / 'chronostrain_runtime.txt')
...
def load_chronostrain_cluster(json_path: Path) -> pd.DataFrame:
    p = Path(json_path)
    with open(p, "rt") as f:
...
def extract_prediction_subset_bf_conditional(
    ...
    posterior_inclusion_p = np.mean(z_samples, axis=0)
    ...
    for tgt_idx, tgt_acc in enumerate(truth_accs):
        cluster_acc = chronostrain_clustering_df.loc[
            chronostrain_clustering_df['Accession'] == tgt_acc, 'Cluster'].item()
...
def chronostrain_results(mut_ratio: str, replicate: int, read_depth: int, trial: int):
    ...
    chronostrain_clustering_df = load_chronostrain_cluster(Path('/mnt/e/ecoli_db/chronostrain_files/ecoli.json'))
    ...
    # Phylogroup-A normalized matrices.
    ...
    auroc = sklearn.metrics.roc_auc_score(  # Abundance thresholding
        ...
        # y_score=np.tile(A_pred_indicators, (len(time_points), 1)).flatten()
