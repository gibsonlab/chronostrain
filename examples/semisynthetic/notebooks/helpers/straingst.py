...
def extract_straingst_prediction(
    ...
    cluster_df = load_straingst_cluster()
    cluster_ordering = sorted(pd.unique(cluster_df['Cluster']))

    output_dir = output_basedir / 'straingst'
    ...
    runtime = 0
    for t_idx in range(len(time_points)):
        runtime += parse_runtime(output_basedir / f'straingst_runtime.{t_idx}.txt')
...
def load_straingst_cluster() -> pd.DataFrame:
...
def straingst_results(mut_ratio: str, replicate: int, read_depth: int, trial: int):
