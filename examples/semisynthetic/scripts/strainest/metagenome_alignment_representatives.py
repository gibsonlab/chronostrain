...
from scipy.cluster.hierarchy import ClusterNode, cut_tree, linkage
import subprocess
...
# def get_leaves(node: ClusterNode) -> List[str]:
#     to_search = [node]
#     found_leaves = []
#     while len(to_search) > 0:
#         next = to_search.pop()
#         if next.is_leaf():
#             found_leaves.append(next.id)
#         if next.left is not None:
#             to_search.append(next.left)
#         if next.right is not None:
#             to_search.append(next.right)
#     return found_leaves
#
#
# def cutoff_tree(root_node: ClusterNode, dist_cutoff: float) -> List[List[str]]:
#     # breadth-first search
#     to_search = [root_node]
#     result = []
#     while len(to_search) > 0:
#         next = to_search.pop()
#         if next.dist < dist_cutoff:
#             result.append(get_leaves(next))
#         else:
#             if next.left is not None:
#                 to_search.append(next.left)
#             if next.right is not None:
...
def main(index_path: Path, work_dir: Path, out_path: Path, n_threads: int):
    ...
    print("Picking cluster representatives.")
