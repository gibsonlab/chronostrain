...
breadcrumb=${output_dir}/mgems.efaecalis.DONE
if ! [ -f ${species_breadcrumb} ]; then
  echo "[*] mGEMS species-binning for ${participant} [Sample ${sample_id}] not yet done."
  exit 1
fi
if [ -f ${breadcrumb} ]; then
  echo "[*] mGEMS efaecalis quantification for ${participant} [Sample ${sample_id}] already done."
  exit 0
fi
...
n_colors=$(cat "${EFAECALIS_REF_CLUSTER}" | wc -l)
aln_and_compress ${strain_fq_1} ${strain_fq_2} ${strain_aln_1} ${strain_aln_2} ${EFAECALIS_REF_INDEX} ${n_colors} ${strain_outdir}/tmp
