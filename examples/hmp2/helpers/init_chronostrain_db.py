...
def download_reference(accession: str, metaphlan_pkl_path: Path, metaphlan_clades: List[str]) -> Dict[str, Path]:
    ...
    for cluster, cluster_genes in get_marker_genes(metaphlan_pkl_path, metaphlan_clades):
...
def get_marker_genes(metaphlan_pkl_path: Path, clades: List[str]) -> Iterator[Tuple[str, List[str]]]:
    db = pickle.load(bz2.open(metaphlan_pkl_path, 'r'))
    u = UniProt()

    for metaphlan_marker_id in itertools.chain(*(
        metaphlan_markers(db, clade),
        for clade in clades
    )):
...
def main():
    ...
    ref_gene_paths = download_reference(args.reference_accession, metaphlan_pkl_path, metaphlan_clades)
